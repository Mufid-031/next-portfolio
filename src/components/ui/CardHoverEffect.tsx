...
export const HoverEffect = ({
  ...
  return (
    <div className={cn(`grid grid-cols-1 ${items.length === 0 ? "md:grid-cols-1 lg:grid-cols-1" : "md:grid-cols-2 lg:grid-cols-3"} py-10`, className)}>
      {items.length === 0 ? (
        ...
        items.map((item, idx) => (
          <Link href={`/blog/${item.slug}`} key={item.slug} className="relative group block p-2 h-full w-full" onMouseEnter={() => setHoveredIndex(idx)} onMouseLeave={() => setHoveredIndex(null)}>
            <AnimatePresence>
              {hoveredIndex === idx && (
                <motion.span
...
export const Card = ({ className, children }: { className?: string; children: React.ReactNode }) => {
  return (
    <div className={cn("rounded-xl h-full w-full overflow-hidden bg-black border border-transparent dark:border-white/[0.2] group-hover:border-slate-700 relative z-20", className)}>
